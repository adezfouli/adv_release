...
def learn(env,
    ...
    **network_kwargs
    ...
    if load_path is not None:
        ...
        logger.logger("Restoring from {}".format(manager.latest_checkpoint))
    ...
    for t in range(total_timesteps):
        ...
        if t % checkpoint_freq == 0:
            save_path = checkpoint_path + 'model-' + str(t) + '.h5'
            logger.log('Trained model saved to: ' + save_path)
        ...
        if done and print_freq is not None and len(episode_rewards) % print_freq == 0:
            logger.record_tabular("date time", str(datetime.datetime.now()))
